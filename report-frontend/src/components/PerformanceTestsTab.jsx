...
const PerformanceTestsTab = ({ register, errors, setActiveTab, getValues, isValid, trigger, setValue, clearErrors }) => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      // Helper function to calculate fallback TQ score based on raw score
      const calculateFallbackTQ = (testName) => {
        const test = tests.find(t => t.name === testName);
        const rawScore = test ? test.raw_score : 0;
        
        if (rawScore > 100) return 160;
        if (rawScore > 80) return 140;
        if (rawScore > 60) return 120;
        return 100;
      };
      ...
      data.results.forEach((item) => {
        if (item && typeof item.tq_score === "number") {
          ...
          // Handle null or invalid TQ score with fallback value based on raw score
          const fallbackTQ = calculateFallbackTQ(item.name);
          
          // Store fallback value in updatedTqScores
          updatedTqScores[item.name] = fallbackTQ;
          
          // Save fallback value to localStorage
          localStorage.setItem(`TQ_${item.name}`, fallbackTQ);
          
          // Add to totalTQ for accurate average calculation
          totalTQ += fallbackTQ;
          count++;
          ...
          if (fieldToError) {
            setError(fieldToError, {
              type: "manual",
              message: "Score exceeded norm table, max value assigned.",
            });
          }
        }
      });
