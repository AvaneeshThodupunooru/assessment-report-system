...
const generateSummary = () => {
  ...
  const iq = parseFloat(localStorage.getItem("OverallTQ"));
  const verbalIQ = parseFloat(localStorage.getItem("verbalIQ"));
  const performanceIQ = parseFloat(localStorage.getItem("performanceIQ"));

  const getIQLevel = (iq) => {
    if (iq < 70) return "low level of intelligence";
    if (iq < 85) return "borderline level of intellectual functioning";
    if (iq <= 115) return "average level of intelligence";
    return "high level of intelligence";
  };

  const level = getIQLevel(iq);

  return `${name} was assessed using standardized psychological tests. The verbal IQ is ${verbalIQ}, and the performance IQ is ${performanceIQ}. The overall IQ is ${iq}, indicating a ${level}. Based on the assessment, appropriate educational interventions are recommended.`;
};
  ...
  return (
    <div className="max-w-full sm:max-w-4xl mx-auto px-4 sm:px-6 md:px-8 space-y-6">
      ...
      <div className="flex flex-col">
        ...
        <button
          type="button"
          className="px-4 py-2 mb-2 rounded bg-blue-600 text-white"
          onClick={() => {
        ...
        <textarea
          id="summary"
          placeholder="Enter summary or click 'Auto Generate Summary'"
          rows={4}
          className="block w-full px-4 py-3 mt-2 text-base sm:text-lg text-gray-900 border rounded-lg focus:outline-none transition-all duration-200 resize-none"
          style={{
            color: "var(--dark-gray)",
