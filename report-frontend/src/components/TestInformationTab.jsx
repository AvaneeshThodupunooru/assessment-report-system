...
const TestInformationTab = ({ register, watch, errors, setActiveTab, isValid, trigger, setValue }) => {
  const tabRef = useRef(null);
  const selectedTest = watch("testsadministered");
    ...
    <div className="space-y-6" ref={tabRef}> {/* Attach ref to the main div */}
      ...
      {/* Other Test */}
        {selectedTest === "other" && (
          ...
          <label
            ...
            className="text-base font-medium" style={{ color: 'var(--text-gray)' }}
          >
          Select Other Test {errors.otherTest && <span className="text-red-500">*</span>}
          </label>
          <input
            id="otherTest"
            type="text"
      ...
      <div className="flex justify-between">
        ...
        <button
          ...
          onClick={async () => {
            const result = await trigger(["testsadministered", 
              selectedTest ==="other"? "otherTest" : null, 
              "readingAge", 
              "spellingAge"]);
            if (result) {
              setActiveTab("tab3");
